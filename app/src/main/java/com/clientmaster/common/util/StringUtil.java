...
public class StringUtil {
	 ...
	 * @exception:
	 * @author: lihy
	 * @time:2015-3-23 下午3:01:12
	 */
	public static SpannableString changeColor(String body, int color) {
		SpannableString spanStr = new SpannableString(body);
		spanStr.setSpan(new ForegroundColorSpan(color), 0, body.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		return spanStr;
	}

	/**
	 * 
	 * @Description:改变颜色并添加点击事件
	 * @param body
	 * @param color
	 ...
	 * @exception:
	 * @author: lihy
	 * @time:2015-3-23 下午3:52:50
	 */
	public static SpannableString addOnClick(String body, ClickableSpan onClickListener) {
		SpannableString spanStr = new SpannableString(body);
		spanStr.setSpan(onClickListener, 0, body.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		// spanStr.setSpan(new ForegroundColorSpan(color), 0, body.length(),
